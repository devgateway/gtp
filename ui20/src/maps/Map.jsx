...
const formatOptions = {
  ...
  State > {
    ...
    getFillColor(d) {
      const colorInterpolator = d3.scaleSequential(d3['interpolate' + this.props.color]);
      if (d.properties[this.props.measure]) {
        return colorInterpolator(d.properties[this.props.measure] / 100)
      } else {
        return '#FFF'
      }
    }
        ...
        const measure = this.props.measure
          const getFillColor = this.getFillColor
            var text1,
              text2;
            if (fid != null) {
              text1 = `${data.properties.NAME_1}`
              text2 = `${measure}: ${this.props.intl.formatNumber(data.properties[measure] / 100, formatOptions)}`

            }
            this.g.selectAll('path').style(
              'fill', (d) => fid && d.properties.fid === fid
              ? getFillColor(d)
              : getFillColor(d)).style(
              'stroke', (d) => fid && d.properties.fid === fid
              ? 'red'
              : '#EEE');
            ...
            if (fid != null) {
              ...
              this.svg.append('rect').attr("rx", 6).attr("ry", 6).attr('x', 4).attr('y', 4).attr('width', width / 2).attr('height', 60)
          ...
          clicked(d) {
            ...
            let fid1 = this.props.selection
              ? this.props.selection.fid
              : null
          ...
          componentDidUpdate(prevProps) {
            ...
            if (this.props.selection) {
              if ((prevProps.selection == null && this.props.selection != null) || (prevProps.selection.fid != this.props.selection.fid1)) {
                this.featureClick(this.props.selection.fid);
              }
            }
          }
          ...
          createPaths(json) {
            this.g.selectAll('path').data(json.features).enter().append('path').attr('d', this.path).attr('vector-effect', 'non-scaling-stroke').style('fill', this.getFillColor).on('click', this.clicked).style('stroke', '#EEE')
          ...
          generate() {
            ...
            var interpolatePercentage = d3.interpolateNumber(0, 100)
