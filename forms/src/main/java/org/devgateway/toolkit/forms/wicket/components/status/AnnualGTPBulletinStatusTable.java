 ...
 */
public class AnnualGTPBulletinStatusTable extends AbstractStatusTable<AnnualGTPBulletinProgress> {

    public AnnualGTPBulletinStatusTable(String id, IModel<AnnualGTPBulletinProgress> model) {
        super(id, model);

        add(new Label("locationLabel", new StringResourceModel("location")));
        add(new Label("statusLabel", new StringResourceModel("status")));

        add(new ListView<DepartmentStatus>("rows", model.map(AnnualGTPBulletinProgress::getDepartmentStatuses)) {
